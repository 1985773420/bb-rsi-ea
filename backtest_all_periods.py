...
def bt_sliding(bars):
    trades=[];pos=None;ep=0;eb=0;n=len(bars)
...
for label,days in [('24小时',1),('30天',30),('90天',90),('180天',180)]:
    sub=all_bars[max(0,n-days*96):]
    trades=bt_sliding(sub)
    if not trades:
        print(label+': 无交易')
        continue
    ...
    to_n=sum(1 for t in trades if t['reason']=='TO')

    print()
    print('='*60)
    print(label+' | window'+str(WINDOW)+' ATR'+str(ATR_F)+' | 无费无滑点')
    print(str(len(trades))+'笔 WR'+str(int(wr))+'% TP'+str(tp_n)+'/SL'+str(sl_n)+'/TO'+str(to_n)+' 纯策略'+('+'if total_pnl>0 else'')+str(round(total_pnl,1))+'%')

    if days<=30:
        daily={}
        for t in trades:
            d=datetime.fromtimestamp(t['ts']/1000).strftime('%m/%d')
            if d not in daily:daily[d]=[]
            daily[d].append(t)
        bal=35.91
        for d in sorted(daily.keys()):
            dt=daily[d];dpnl=sum(t['pnl']for t in dt);dw=[t for t in dt if t['pnl']>0]
            for t in dt:bal*=(1+t['pnl']*MARGIN*LEV)
            print(d+' '+str(len(dt))+'t WR'+str(int(len(dw)/len(dt)*100))+'% PnL'+('+'if dpnl>0 else'')+str(round(dpnl*100,1))+'% Bal $'+str(round(bal,2)))
            for t in dt:
                et=datetime.fromtimestamp(all_bars[min(t['eb'],len(all_bars)-1)]['ts']/1000).strftime('%H:%M')
                xt=datetime.fromtimestamp(t['ts']/1000).strftime('%H:%M')
                c='+'if t['pnl']>0 else''
                print('  '+et+'-'+xt+' '+t['side']+' '+c+str(round(t['pnl']*100,2))+'% ['+t['reason']+']')
        ...
        for m in sorted(monthly.keys()):
            ...
            tp_m=sum(1 for t in mt if t['reason']=='TP')
            sl_m=sum(1 for t in mt if t['reason']=='SL')
            print()
            print(m+' '+str(len(mt))+'t WR'+str(int(mw/len(mt)*100))+'% PnL'+('+'if mp>0 else'')+str(round(mp*100,1))+'% Bal $'+str(round(bal,2))+' TP'+str(tp_m)+'/SL'+str(sl_m))
            for t in mt[:2]:
